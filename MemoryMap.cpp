...
char& MemoryMap::operator[](const size_t index) {
	if (index > fileSize()) {
		throw std::out_of_range("Index out of range");
	}

	int page_index = getPageOnIndex(index);



	return m_map[page_index].buffer[index - m_map[page_index].from];
}
...
std::pair<size_t, size_t> MemoryMap::getPageRange(size_t byteIndex) {
	...
	size_t to = file_size > from + PAGE_SIZE - 1 ? from + PAGE_SIZE - 1 : from + file_size - from - 1;
